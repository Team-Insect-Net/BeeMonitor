...
def saveFrame(frame, frame_number, video_path):
...
def saveAnnotatedFrame(annotated_frame, frame_number, video_path):
    ''' Save a single annotated frame as an image file '''
...
def getNestDetection(video_path, nest_model, res_height=360, res_width=640):
    ...
    while len(nest_detections[0]) < 60:
        # frame_counter += 1
        ...
        if success:
            ...
            #saveFrame(frame, frame_counter, video_path)


            # Visualize the results on the frame
            #annotated_frame = results[0].plot()
            #saveAnnotatedFrame(annotated_frame, frame_counter, video_path)
...
def plotIDsOnFrame(frame, nest_ids):
    ''' Plot nest IDs on the given frame '''
    for nest_id, coords in nest_ids["nests"].items():
        x1, y1, x2, y2 = coords
        cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
        cv2.putText(frame, nest_id, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    return frame


def processNestDetection(video_path, nest, res_height=360, res_width=640):
    ...
    for j in range(len(fixed_dl_rows)):
        ...
        for i, hole in enumerate(sorted_row):
            nest_ids["nests"][f'{(i+1) + j * 10}'] = (hole[0] - width//2, hole[1] - height//2, hole[0] + width//2, hole[1] + height//2)

    # frame_raw = plotIDsOnFrame(np.zeros((res_height, res_width, 3), dtype=np.uint8), nest_ids)
    # video_name = video_path.replace('.mp4', '_nest_ids_visualization')
    # saveFrame(frame_raw, 0, video_name)

    # load read frame use video path
    frame = video_path.replace('.mp4', '_frames') + f"_frame_{0:05d}.png"
    frame_img = cv2.imread(frame)
    frame_annotated = plotIDsOnFrame(frame_img, nest_ids)
    saveAnnotatedFrame(frame_annotated, 10000, video_path)
